...
class ProductController extends Controller
{
    ...
    public function productData(Request $request)
    {
        ...
        else
        {
            ...
            $q = Product::select('products.*')
                ...
                //->leftjoin('suppliers', 'products.supplier_id', '=', 'supplier.id')
                ->where([
        ...
        if(!empty($products))
        {
            foreach ($products as $key=>$product)
            {
                ...
                if($volume == 0){
                    ...
                    $nestedData['volume'] = $volume;
                }
                
                // $sup = Supplier::select('name')
                // ->where([
                //     ['id', $product->supplier_id]
                // ])->get();
                // $sup_name = json_encode($sup);
                // $sup_name = str_replace('[{"name":"',"",$sup_name);
                // $sup_name = str_replace('"}]',"",$sup_name);
                // $nestedData['supplier'] = $sup_name;

                $nestedData['supplier'] = $product->supplier_id;

                //$nestedData['supplier'] = Product_Supplier::select('id')->find($product->supplier_id)->id;

                // if($product->supplier_id>0){
                //     $nestedData['supplier'] = Supplier::where([['id', $product->supplier_id] ])->name;
                // } else {
    ...
    public function create()
    {
        ...
        if ($role->hasPermissionTo('products-add')){
            ...
            return view('backend.product.create',compact('lims_supplier_list', 'lims_product_list_without_variant', 'lims_product_list_with_variant', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_warehouse_list', 'numberOfProduct', 'custom_fields'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function edit($id)
    {
        ...
        if ($role->hasPermissionTo('products-edit')) {
            ...
            if(in_array('ecommerce', explode(',',config('addons')))) {
                ...
                return view('backend.product.edit',compact('lims_supplier_list','related_products','lims_product_list_without_variant', 'lims_product_list_with_variant', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_product_data', 'lims_product_variant_data', 'lims_warehouse_list', 'noOfVariantValue', 'custom_fields'));
            }
            return view('backend.product.edit',compact('lims_supplier_list','lims_product_list_without_variant', 'lims_product_list_with_variant', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_product_data', 'lims_product_variant_data', 'lims_warehouse_list', 'noOfVariantValue', 'custom_fields'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
